...
export default function WalletPage() {
    ...
    // Show success toast if redirected back from Lemon Squeezy
    useEffect(() => {
    ...
    const handleTopUp = async () => {
        if (!selectedAmount) {
            setCheckoutError('Please select an amount.');
            return;
        }
            ...
            {showTopUpModal && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white rounded-lg p-6 w-full max-w-md">
                        <h3 className="text-lg font-semibold text-gray-900 mb-1">Top Up Wallet</h3>
                        <p className="text-sm text-gray-600 mb-6">Select an amount to add to your wallet via Lemon Squeezy</p>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    {[10, 50].map((amt) => (
                                        <button
                                            key={amt}
                                            onClick={() => { setSelectedAmount(amt); setCheckoutError(null); }}
                                            className={`px-4 py-3 rounded-lg border-2 text-sm font-medium transition-colors ${selectedAmount === amt
                            ...
                            <p className="text-xs text-gray-500">
                                You will be redirected to Lemon Squeezy to complete your payment securely.
                            </p>
                        </div>

                        <div className="flex gap-3 mt-6">
                            <button
                                onClick={() => { setShowTopUpModal(false); setSelectedAmount(null); setCheckoutError(null); }}
                            ...
                            <button
                                onClick={handleTopUp}
                                disabled={isCheckingOut || !selectedAmount}

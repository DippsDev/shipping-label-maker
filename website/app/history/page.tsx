...
async function fetchLabelBlob(label: Label): Promise<Blob> {
    const response = await fetch('/api/generate-label', {
        ...
        body: JSON.stringify({
            ...
            shipToName: label.recipientName,
            shipToAddress: label.recipientAddress,
            shipToCity: label.recipientCity,
            shipToState: label.recipientState,
            shipToZip: label.recipientZip,
            weight: label.weight,
        }),
    });
...
export default function HistoryPage() {
    ...
    const [viewingLabel, setViewingLabel] = useState<{ url: string; trackingNumber: string } | null>(null);
    ...
    const handleView = async (label: Label) => {
        setActionLoading(`view-${label.id}`);
        try {
            const blob = await fetchLabelBlob(label);
            const url = URL.createObjectURL(blob);
            setViewingLabel({ url, trackingNumber: label.trackingNumber });
        } catch {
            alert('Failed to load label preview.');
        } finally {
    ...
    const handleDownload = async (label: Label) => {
        setActionLoading(`download-${label.id}`);
        try {
            const blob = await fetchLabelBlob(label);
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url;
            a.download = `label_${label.trackingNumber}.png`;
    ...
    const handlePrint = async (label: Label) => {
        setActionLoading(`print-${label.id}`);
        try {
            const blob = await fetchLabelBlob(label);
            const url = URL.createObjectURL(blob);
                    ...
                    <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] flex flex-col">
                        <div className="flex items-center justify-between p-4 border-b border-gray-200">
                            <div>
                                ...
                                <p className="text-sm text-gray-500">{viewingLabel.trackingNumber}</p>
                            </div>
                            <div className="flex items-center gap-2">
                                <button
                                    onClick={() => {
                                        const a = document.createElement('a');
                                        a.href = viewingLabel.url;
                                        a.download = `label_${viewingLabel.trackingNumber}.png`;
                                        a.click();
                                    }}
                                ...
                                <button
                                    onClick={() => {
                                        URL.revokeObjectURL(viewingLabel.url);
                        ...
                        <div className="overflow-auto p-4 flex-1 flex items-center justify-center bg-gray-50">
                            ...
                            <img
                                src={viewingLabel.url}
                                alt={`Label ${viewingLabel.trackingNumber}`}
